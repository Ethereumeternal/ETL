...
{
    resize(850, 550);
    setWindowTitle(tr("InnovationIntegral") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a InnovationIntegral address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About InnovationIntegral"), this);
    aboutAction->setToolTip(tr("Show information about InnovationIntegral"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for InnovationIntegral"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("InnovationIntegral client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("InnovationIntegral client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to InnovationIntegral network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid InnovationIntegral address or malformed URI parameters."));
    }
